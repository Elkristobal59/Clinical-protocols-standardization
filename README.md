...
- **Collecte Intelligente (JSON vs PDF)** : Lorsqu'une pathologie est cherchée, l'app interroge l'API officielle ClinicalTrials V2. 
  ...
  - *Fallback (Plan B)* : Si le texte est manquant, l'application utilise l'API v2 officielle pour récupérer et télécharger le PDF directement via le CDN de ClinicalTrials.gov (ultra-rapide et robuste).
    > **💡 Point d'Ingénierie Fort (Scraping CDN vs Playwright) :** Remplacement stratégique du scraping par navigateur (Playwright) par une interrogation directe et intelligente des endpoints CDN via l'API v2. Résultat : temps de téléchargement divisés par 10, zéro dépendance complexe (navigateurs headless), et fiabilité totale sur des serveurs Serverless/Cloud comme Render.
